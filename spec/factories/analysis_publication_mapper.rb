FactoryBot.define do
  factory :analysis_publication_mapper do
    let(:context_attr) { "@context" }
    let(:type_attr) { "@type" }
    let(:id_attr) { "@id" }

    trait :with_all_args do
      context_attr { "https://schema.org/" }
      type_attr { "AnalysisEntity" }
      id_attr { "https://dx.doi.org/10.14272/MWJHDSAAGSURCA-UHFFFAOYSA-N/CHMO0000595" }
